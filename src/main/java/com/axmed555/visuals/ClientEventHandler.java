...
public class ClientEventHandler {
    ...
    public static void onRenderLevelStage(RenderLevelStageEvent event) {
        ...
        if (Config.HAT_STYLE.get() != Config.HatStyle.NONE) {
            renderHat(player, cameraPos);
        }
        
        if (Config.SHOW_TRAJECTORIES.get()) {
            renderTrajectories(player, cameraPos);
        }
    ...
    private static void renderEnlargedHitbox(Player player, PoseStack poseStack, Vec3 cameraPos) {
        ...
        // Исправленный рендеринг хитбокса
    ...
    private static void renderHat(Player player, Vec3 cameraPos) {
    ...
    private static void renderTrajectories(Player player, Vec3 cameraPos) {
